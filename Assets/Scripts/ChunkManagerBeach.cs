...
public class ChunkManagerBeach : MonoBehaviour
{
    ...
    Dictionary<Vector2, TerrainChunkBeach> terrainChunkDictionary = new Dictionary<Vector2, TerrainChunkBeach>();
    List<TerrainChunkBeach> terrainChunksVisibleLastUpdate = new List<TerrainChunkBeach>();

    public static ChunkManagerBeach instance;
    private void Awake()
    {
        if (player == null)
        {
    ...
    void UpdateVisibleChunks()
    {
        ...
        float currentChunkCoordX = (playerPos.x / chunkSize);
        float currentChunkCoordY = (playerPos.y / chunkSize);

        for (float yOffset = -chunksVisibleInViewDist; yOffset <= chunksVisibleInViewDist; yOffset++)
        {
            for (float xOffset = -chunksVisibleInViewDist; xOffset <= chunksVisibleInViewDist; xOffset++)
            {
                ...
                {
                    terrainChunkDictionary.Add(viewedChunkCoord, new TerrainChunkBeach(viewedChunkCoord, chunkSize, transform));
                }
            }
        }
    }

    public class TerrainChunkBeach
    {
        GameObject meshObject;
        Vector2 position;
        Bounds bounds;

        public TerrainChunkBeach(Vector2 coord, int size, Transform parent)
        {
            position = coord * size;
            bounds = new Bounds(position, Vector2.one * size);
            Vector3 positionV3 = new Vector3(position.x, 0, position.y);

            meshObject = Instantiate(ChunkManagerBeach.instance.chunksAvailable[0], positionV3, Quaternion.identity);
            meshObject.transform.position = new Vector3(positionV3.x, positionV3.y, positionV3.z - 6.75f); 
